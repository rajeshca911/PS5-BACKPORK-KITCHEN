...
class SignedElfNpdrmControlBlock(object):
    ...
    def __init__(self):
        self.type = SELF_CONTROL_BLOCK_TYPE_NPDRM  # Assuming this is defined elsewhere as an integer
        self.content_id = None
        self.random_pad = None

    def save(self, f):
        # Ensure that content_id and random_pad are bytes
        if isinstance(self.content_id, str):
            content_id_bytes = self.content_id.encode('utf-8')
        else:
            content_id_bytes = self.content_id  # Assuming it's already bytes

        if isinstance(self.random_pad, str):
            random_pad_bytes = self.random_pad.encode('utf-8')
        else:
            random_pad_bytes = self.random_pad  # Assuming it's already bytes

        # Pack the data
        packed_data = struct.pack(self.FMT, self.type, content_id_bytes, random_pad_bytes)
        f.write(packed_data)
...
class SignedElfFile(object):
    COMMON_HEADER_FMT = b'<4s4B'
    EXT_HEADER_FMT = b'<I2HQ2H4x'
    ...
    def _prepare(self):
        ...
        if not self.auth_info is None:
            self.signature = (struct.pack(b'<QQ', len(self.auth_info), self.ex_info.paid) + self.auth_info[8:]).ljust(SIGNATURE_SIZE, '\0')
        else:
    ...
    def save(self, f):
        ...
        # Ensure self.signature is a bytes object
        if isinstance(self.signature, str):
            signature_bytes = self.signature.encode('utf-8')
        else:
            signature_bytes = self.signature  # Assuming it's already bytes
        ...
        for entry in self.entries:
            ...
            if isinstance(entry.data, str):
                ...
                data_bytes = entry.data  # Assuming it's already bytes

            f.write(data_bytes)

        # Ensure self.signature is a bytes object
        if isinstance(self.version_data, str):
            version_data_bytes = self.version_data.encode('utf-8')
        else:
            version_data_bytes = self.version_data  # Assuming it's already bytes

        # write version
        if not self.version_data is None:
            f.write(version_data_bytes)
