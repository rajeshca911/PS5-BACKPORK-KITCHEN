...
Module selfutilmodule
    ...
    Public Function Getselfutilexepath() As String
        ...
        Return selfutilexe
    End Function
    Public Function Getmyselfutilexepath() As String
        Dim selfutilexe As String = Path.Combine(selfutilpath, "selfutil.exe")
        Return selfutilexe
    End Function

    'Public Function unpackfile(sourcefile As String, outputElfPath As String, Optional UseMyselfutil As Boolean = True) As Boolean
    '    Try

    '        Dim selfutilexe As String = Getmyselfutilexepath()
    ...
    '        ' Build arguments (simple decrypt mode)
    '        Dim args As New List(Of String)
    '        args.Add($"""{sourcefile}""")
    '        'args.Add("--rebuild")   ' only if needed
    '        'args.Add("--debug")
    '        args.Add($"--output ""{outputElfPath}""")
    '        Dim argumentString As String = String.Join(" ", args)

    '        If Not UseMyselfutil Then
    '            selfutilexe = Getselfutilexepath()
    '            argumentString = $"--verbose --overwrite --input ""{sourcefile}"" --output ""{outputElfPath}"""

    '        End If
    '        'MessageBox.Show(selfutilexe)

    '        Dim startInfo As New ProcessStartInfo() With {
    '        .FileName = selfutilexe,
    '        .Arguments = argumentString,'$"--verbose --overwrite --input ""{sourcefile}"" --output ""{outputElfPath}""",
    ...
    '            Logger.Log(Form1.rtbStatus, e.Data, Color.Blue)
    '        End If
    '    End Sub
    ...
    '    Sub(sender, e)
    '        If e.Data IsNot Nothing Then
    '            Logger.Log(Form1.rtbStatus, e.Data, Color.Red)
    '        End If
    '    End Sub
    ...
    Public Function GetFileType(data As Byte()) As String
        ...
        If magic = ELF_MAGIC Then
            Return "ELF"
        End If
        'CURRENTLY CHECK EITHER ELF OR NOT
        '' 2️⃣ SELF
